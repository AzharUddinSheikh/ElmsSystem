...
if (isset($_GET['document'])) {
    ...
    $id = base64_decode($_GET['document']);
    $userid = $leave_request->userid($id);
    $documentAskStatus = 1 ;
    $type = 1 ;
    $message = "Document Require For Leave";
    $enterRequest = $notification->enterRequest($id, $documentAskStatus, $userid, $type, $message);
    $updateRequestInLeave = $notification->updateRequestInLeave($id);
    $_SESSION["message"] = "Message Dend To Upload Document";
    header("Location:http://localhost/ElmsSystem-sahil/twig/admin.php");
...
echo $template->render(['leaves' => $leaves, 'count' => sizeof($leaves), 'cyear' => $cyear , 'displayNumber' => $displayNumber ]);
?>